...
void MTIMER1_voidInit()
{
    ...
    #elif     TIMER1_Mode == TIMER1_AS_DELAY
	/*Choose Delay Mode*/
		CLR_BIT(TCCR1A, TCCR1A_WGM10);
		CLR_BIT(TCCR1A, TCCR1A_WGM11);
		SET_BIT(TCCR1B, TCCR1B_WGM12);
		...
		/*Set compare match value to 1000*/
		OCR1A=1000;
...
void MTIMER1_voidSetBusyWait(u16 Copy_u16DelayTime)
{
		...
		u8 CHECK_FLAG=0;
		while(Local_u16Counter!=Copy_u16DelayTime)
		{
			CHECK_FLAG=GET_BIT(TIFR,TIMSK_OCIF1A);
			if(CHECK_FLAG==1)
			{

				Local_u16Counter++;
				SET_BIT(TIFR,TIMSK_OCIF1A);
				//TCNT1=0;
			}
		}
}
